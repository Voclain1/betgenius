...
type Leg = { legId: string; matchLabel: string; market: string; pick: string; predictionId: string | null };
...
export default function EditCombo({ params }: { params: { id: string } }) {
  ...
  const [draft, setDraft] = useState({ matchLabel: "", market: "1X2", pick: "Home" });
  ...
  const addFromTip = (opt: TipOption) => {
    ...
    setLegs((prev) => [...prev, { legId: crypto.randomUUID(), matchLabel: opt.label, market: opt.market, pick: opt.pick, predictionId: opt.id }]);
  };

  const addManual = () => {
    if (!draft.matchLabel) return;
    setLegs((prev) => [...prev, { legId: crypto.randomUUID(), matchLabel: draft.matchLabel, market: draft.market, pick: draft.pick, predictionId: null }]);
    ...
    try {
      const res = await fetch(`/api/admin/combos/${params.id}`, {
        ...
        body: JSON.stringify({
          ...
          legs: legs.map((l) => ({ matchLabel: l.matchLabel, market: l.market, pick: l.pick, predictionId: l.predictionId })),
        }),
      });
